...
class TransfertController extends Controller
{
    ...
    public function index()
    {
        ...
        $comptes = Compte::all();
        $transactions = Transfert::with('compte')->latest()->paginate(10);
...
public function deposit(Request $request)
{
    ...
    public function cancel($id)
    {
        ...
        if ($transaction->status === Transfert::STATUS_CANCELLED) {
        ...
        DB::transaction(function () use ($transaction, $compte) {
            ...
            $transaction->status = Transfert::STATUS_CANCELLED; // Utilisez la constante définie
            $transaction->save();
        });
