...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $transactionsJour = Transaction::whereDate('created_at', today())->count();

        // Calculer le volume total des transactions complètes du jour
        $volumeJour = Transaction::whereDate('created_at', today())
            ->where('status', 'completed')
            ->sum('montant');

        // Récupérer les dernières transactions sans expéditeurs et destinataires
        $dernieresTransactions = Transaction::latest()
            ->take(10)
            ->get();
