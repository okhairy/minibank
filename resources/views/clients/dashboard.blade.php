        ...
        <div class="col-md-8">
            ...
            <div class="card" style="border-radius: 15px; position: fixed; top: 40px; left: 50%; transform: translateX(-50%); z-index: 1000; width: 80%; max-width: 850px;">
                <div class="card-header text-center" style="background-color: #1C627B; color: white; padding: 50px; border-radius: 15px 15px 0 0; position: relative;">
                    ...
                    <h2>
                        <span id="solde" style="font-size: 26px"> {{ number_format($solde) }}F</span>
                    ...
                    <div class="text-center" style="padding: 20px;">
                      ...
                      <!-- Affichage du QR code -->
                        <img src="{{ $qrCodeUrl }}" alt="QR Code" style="border-radius: 10px; width: 200px; height: 150px; border: 10px solid #FFFFFF;" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-content">
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <form action="{{ route('clients.transfer') }}" method="POST">
                            @csrf
...
@endsection
<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js">document.addEventListener('DOMContentLoaded', function() {
  var qr = new QRious({
...
@section('scripts')
<script>
    // Gestion de l'affichage du menu des paramètres
    ...
    document.getElementById('toggleSolde').addEventListener('click', function() {
        var soldeSpan = document.getElementById('solde');
        var soldeIcon = document.getElementById('soldeIcon');

        if (soldeSpan.style.display === 'none') {
            soldeSpan.style.display = 'block';
            soldeIcon.className = 'fa fa-eye';
        } else {
            soldeSpan.style.display = 'none';
            soldeIcon.className = 'fa fa-eye-slash';
        }
    });

    // Affichage de la modale d'erreur si elle est présente
    @if ($errors->any())
        $('#errorModal').modal('show');
    @endif

    // Affichage de la modale de transfert
